...
const CheckoutPage = () => {
  ...
  const loadSavedCardInfo = async () => {
    try {
      ...
      const response = await fetch(getApiUrl(`/api/get-card?userEmail=${user?.useremail}`));
      const data = await response.json();
  ...
  const loadShipmentAddress = async () => {
    ...
    try {
      const response = await fetch(
        getApiUrl(`/api/shipment-address/${user.useremail}`)
      );
      const data = await response.json();
  ...
  const handleSaveCard = async () => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl('/api/save-card'), {
        method: 'POST',
        headers: {
  ...
  const saveOrderToDatabase = async () => {
    try {
      ...
      const response = await fetch(getApiUrl('/api/save-order'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className="flex flex-col items-center p-2">
                    ...
                    <span className="text-xs text-gray-600">7 days to return</span>
                  </div>
                  <div className="flex flex-col items-center p-2">
