...
import { MdCable, MdOutlineScreenshotMonitor } from "react-icons/md";
...
// Function to get appropriate icon for included items
const getIncludedItemIcon = (item) => {
  const lowerItem = item.toLowerCase();

  // Laptop related
  if (
    lowerItem.includes("laptop") ||
    lowerItem.includes("dell") ||
    lowerItem.includes("lenovo") ||
    lowerItem.includes("hp") ||
    lowerItem.includes("acer") ||
    lowerItem.includes("asus")
  ) {
    return <FaLaptop className="text-gray-700 text-lg" />;
  }

  // Desktop related
  if (
    lowerItem.includes("desktop") ||
    lowerItem.includes("pc") ||
    lowerItem.includes("computer")
  ) {
    return <FaDesktop className="text-gray-700 text-lg" />;
  }

  // Tablet related
  if (lowerItem.includes("tablet") || lowerItem.includes("ipad")) {
    return <FaTabletAlt className="text-gray-700 text-lg" />;
  }
  ...
  if (
    ...
    lowerItem.includes("power")
  ) {
    return <ImPower className="text-gray-700 text-lg" />;
  }

  // Windows OS
  if (
    lowerItem.includes("windows") ||
    lowerItem.includes("os") ||
    lowerItem.includes("operating system")
  ) {
    return <FaWindows className="text-gray-700 text-lg" />;
  }

  // Security/Antivirus
  if (
    lowerItem.includes("defender") ||
    lowerItem.includes("antivirus") ||
    lowerItem.includes("security")
  ) {
    return <FaShieldAlt className="text-gray-700 text-lg" />;
  }

  // Keyboard
  if (lowerItem.includes("keyboard")) {
    return <FaKeyboard className="text-gray-700 text-lg" />;
  }

  // Mouse
  if (lowerItem.includes("mouse")) {
    return <FaMouse className="text-gray-700 text-lg" />;
  }

  // Printer
  if (lowerItem.includes("printer") || lowerItem.includes("print")) {
    return <FaPrint className="text-gray-700 text-lg" />;
  }

  // Network
  if (lowerItem.includes("network") || lowerItem.includes("ethernet")) {
    return <FaNetworkWired className="text-gray-700 text-lg" />;
  }

  // Storage
  if (
    lowerItem.includes("ssd") ||
    lowerItem.includes("hdd") ||
    lowerItem.includes("storage") ||
    lowerItem.includes("drive") ||
    lowerItem.includes("ram")
  ) {
    return <FaHdd className="text-gray-700 text-lg" />;
  }
  ...
  if (
    ...
    lowerItem.includes("audio")
  ) {
    return <FaVolumeUp className="text-gray-700 text-lg" />;
  }

  // CPU/Processor
  if (
    lowerItem.includes("cpu") ||
    lowerItem.includes("processor") ||
    lowerItem.includes("intel") ||
    lowerItem.includes("amd")
  ) {
    return <FaMicrochip className="text-gray-700 text-lg" />;
  }

  // WiFi
  if (lowerItem.includes("wifi") || lowerItem.includes("wireless")) {
    return <FaWifi className="text-gray-700 text-lg" />;
  }

  // Bluetooth
  if (lowerItem.includes("bluetooth")) {
    return <FaBluetooth className="text-gray-700 text-lg" />;
  }

  // Camera
  if (lowerItem.includes("camera") || lowerItem.includes("webcam")) {
    return <FaCamera className="text-gray-700 text-lg" />;
  }

  // Screen
  if (lowerItem.includes("screen")) {
    return <MdOutlineScreenshotMonitor className="text-gray-700 text-lg" />;
  }

  // Motherboard
  if (
    lowerItem.includes("motherboard") ||
    lowerItem.includes("board") ||
    lowerItem.includes("mother")
  ) {
    return <BsMotherboard className="text-gray-700 text-lg" />;
  }

  // Default icon for unknown items
  return <FiPackage className="text-gray-700 text-lg" />;
};
...
// const whatsIncluded = [
//   {
//     icon: <FaKeyboard className="text-gray-700 text-3xl mx-auto" />,
//     label: "Charger",
//   },
//   {
//     icon: <FaLaptop className="text-gray-700 text-3xl mx-auto" />,
//     label: "Laptop",
//   },
// ];
...
const ProductPage = () => {
  ...
  const getScreenSizeOptions = () => {
    if (product?.ProductCategory === "Laptop") {
      ...
      product?.ProductCategory === "Monitors"
    ) {
  ...
  const getFieldValues = () => {
    ...
    if (product.ProductCategory === "Mouse") {
      ...
      return {
        field1: product.MonitorPanel || product.ProductKeyboard, // Panel Type (IPS, LCD, LED, etc.)
        field2: product.ProductResolution || product.ProductRam, // Resolution (1920x1080, etc.)
        field3: product.MonitorSize || product.ProductScreenSize, // Screen Size (23", 24", etc.)
        field4: product.MonitorRefreshRate || product.ProductStorage, // Refresh Rate (60Hz, etc.)
      };
    } else if (product.ProductCategory === "Drive") {
      return {
  ...
  const getTechnicalSpecs = () => {
    ...
    if (
      ...
      // Monitor-specific specifications using correct database fields
      if (product.MonitorPanel)
        specs.push({ label: "Panel Type", value: product.MonitorPanel });
      if (product.ProductResolution)
        specs.push({ label: "Resolution", value: product.ProductResolution });
      if (product.MonitorSize)
        specs.push({ label: "Screen Size", value: product.MonitorSize });
      if (product.MonitorRefreshRate)
        specs.push({ label: "Refresh Rate", value: product.MonitorRefreshRate });
      if (product.ProductWeight)
        specs.push({ label: "Weight", value: product.ProductWeight });
      // Fallback to other fields if monitor-specific fields are not available
      if (!product.MonitorPanel && product.ProductKeyboard)
        specs.push({ label: "Panel Type", value: product.ProductKeyboard });
      if (!product.MonitorSize && product.ProductScreenSize)
        specs.push({ label: "Screen Size", value: product.ProductScreenSize });
      if (!product.MonitorRefreshRate && product.ProductStorage)
        specs.push({ label: "Refresh Rate", value: product.ProductStorage });
    } else if (product.ProductCategory === "Mouse") {
                    ...
                    {isFromAuction && (
                      <div className="bg-[#f5f5f5] rounded-lg p-4">
                        <div className="flex flex-col items-center">
                          <FiPackage className="text-gray-700 text-2xl md:text-3xl mx-auto mb-2" />
                          <h2 className="text-2xl font-bold text-gray-900 mb-4">
                            What's Included
                          </h2>
                        </div>
                        <div className="bg-white border border-gray-300 rounded-lg p-4 max-w-xl mx-auto shadow-sm text-sm space-y-3 text-gray-800">
                          <div className="space-y-3">
                            {typeof product.included_items === "string" &&
